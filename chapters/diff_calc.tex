...
\begin{proof}
	...
	For $n > 1$, assume $(x^n)' = nx^{n-1}$. Then by Equation \ref{eq:prod_rule}, since $x^{n+1} = x\cdot x^n$, we have
	\[
		(x^{n+1})' = (x \cdot x^n)' = 1 \cdot x^n + x \cdot n x^{n-1} = (n + 1)x^n.
	\]
	This proves the inductive step and establishes the result. Finally, the linearity of the derivative (see Equation \ref{eq:sum_rule}) yields the differentiability of any polynomial. \qedhere
\end{proof}
...
\begin{definition}{Convex Functions}{convex_func}
	...
	\end{equation}
	We call $f$ \textbf{strictly convex} if the inequality in \ref{eq:convex_func} is strict. A function $g:I \to \mathbb{R}$ is (\textbf{strictly}) \textbf{concave} if $-g$ is (strictly) convex.
...
\begin{proof}
	...
	\]
	Since $f'$ is increasing, we have $f'(\xi) \leq f'(\zeta)$, so Equation \ref{eq:convex_func_2} follows. Since $a < b \in I$ and $x \in (a, b)$ are arbitrary, $f$ is convex.
	
	Conversely, assume $f$ is convex. Given $a < b$, consider $h > 0$ small enough, such that $a + h < b - h$ and apply Equation \ref{eq:convex_func_2} twice: first, applying it on the interval $(a, b - h)$ with $x = a + h$ we get
	\[
