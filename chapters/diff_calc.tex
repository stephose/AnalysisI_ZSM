...
\begin{proof}
	...
	therefore
	\begin{align*}
		 \lim_{x \to x_0} \frac{g(f(x)) - g(f(x_0))}{x - x_0} &= \lim_{x \to x_0} \bigg( g'(f(x_0))\frac{f(x) - f(x_0)}{x - x_0} + \omega(f(x)) \frac{f(x) - f(x_0)}{x - x_0} \bigg)\\
		 &= g'(f(x_0)) f'(x_0) + \underbrace{\omega(f(x_0))}_{= 0} f'(x_0) = g'(f(x_0)) f'(x_0),
	\end{align*}
	where we used the continuity of $\omega$ at $y_0 = f(x_0)$ to deduce that $\omega(f(x)) \to \omega(f(x_0)) = 0$ as $x \to x_0$. \qedhere
\end{proof}
...
\begin{proof}
	...
	\]
	By the continuity of $f^{-1}$ we have $f^{-1}(y_n) = x_n \to \bar{x} = f^{-1}(\bar{y})$. Thus, since $f$ is differentiable at $\bar{x}$ with $f'(\bar{x}) \neq 0$, Proposition \ref{prop*lim_op}(4) implies
	\[
...
\begin{corollary}{Lipschitz Continuity vs. Bounded Derivative}{lip}
