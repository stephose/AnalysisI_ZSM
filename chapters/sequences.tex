...
\begin{proof}
	...
	So it follows that $n_{k+1} \geq n_k + 1 \geq k + 1$, which proves the condition for $k+1$.\qedhere
\end{proof}
...
\begin{proposition}{Subsequences and Accumulation Points}{subseq_acc_pt}
	Let $(x_n)_{n=0}^{\infty}$ be a sequence in $\mathbb{R}$. A point $A \in \mathbb{R}$ is an accumulation point of $(x_n)_{n=0}^{\infty}$ if and only if there exists a convergent subsequence of $(x_n)_{n=0}^{\infty}$ with limit $A$.
\end{proposition}
...
\]
The concept of limits can be restated using the tails of a sequence, i.e., the sequence $(x_n)_{n=0}^{\infty}$ converges to $A \in \mathbb{R}$ if and only if, for every $\varepsilon > 0$ there exists $N \in \mathbb{N}$ such that $X_{\geq N} \subseteq (A - \varepsilon, A + \varepsilon)$.
...
\begin{definition}{Superior and Inferior Limits}{sup_inf}
	...
	From Equation \eqref{eq:ineq_sup_inf} and Proposition \ref{prop*lim_ineq}, we have
	\[
...
\begin{proof}
	...
	First suppose that $I = S$. Since $i_n \leq x_n \leq s_n$ (see Equation \eqref{eq:ineq_sup_inf}), the Sandwich Lemma \ref{lem*sandwich_seq} implies that the sequence $(x_n)_{n=0}^{\infty}$ converges, and its limit equals $I = S$.
...
\begin{proof}
	...
	\]
	Thus, combining this bound with Equation \eqref{eq:limsup} we obtain
	\[
	...
	We now prove$1.$ and $2.$. From Equation \eqref{eq:limsup} we have $x_n < A + \varepsilon$ for all $n \geq N_0$, so only finitely many terms satisfy $x_n \geq A + \varepsilon$. This shows $1.$.
