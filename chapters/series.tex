...
\begin{proof}
	...
	\]
	which proves the identity \ref{eq:reordered_series}. Applying the same reasoning to $\sum_{n=0}^{\infty} |a_n|$ shows that $\sum_{\ell = 0}^{\infty} |a_{\varphi(\ell)}| = \sum_{n=0}^{\infty} |a_n| < \infty$, hence $\sum_{\ell = 0}^{\infty} a_{\varphi(\ell)}$ is absolutely convergent.
	\qedhere
\end{proof}
...
\begin{proof}
	...
	\end{align*}
	which proves Equation \ref{eq:product_series} for this specific bijection $\alpha$.
...
\begin{corollary}{Cauchy Product}{cauchy_prod}
	If $\sum_{n=0}^{\infty} a_n$ and $\sum_{n=0}^{\infty} b_n$ are convergent, then
	\[
...
\begin{theorem}{Convergence of Power Series}{conv_pwr_series}
	Let $\sum_{n=0}^{\infty}a_n x^n$ be a power series with radius of convergence $R \in (0, \infty]$. Then $\sum_{n=0}^{\infty}$ converges absolutely for all $x \in \mathbb{R}$ with $|x| < R$, and does not converge for all $x \in \mathbb{R}$ with $|x| > R$. In particular, for $x \in (-R, R)$, we can define the function $f(x) = \sum_{n=0}^{\infty} a_n x^n$.
\end{theorem}
...
\end{equation}
Since $\frac{n!}{(n+1)!} = \frac{1}{(n+1)} \to 0$ as $n \to \infty$, it follows directly from the quotient criterion that this series has infinite radius of convergence. Hence, Theorem \ref{theo*cont_pwr_series} implies that the right-hand side of Equation \ref{eq:exp} defines a continuous function on $\mathbb{R}$.
...
As for the exponential series, radius of convergence of the power series in Equation \ref{eq:sin_cos} is infinite. Therefore, by Theorems \ref{theo*conv_pwr_series} and \ref{theo*cont_pwr_series}, $\sin$ and $\cos$ are continuous on $\mathbb{R}$.

Since $(-x)^{2n+1} = -x^{2n+1}$ and $(-x)^{2n} = x^{2n}$ for all $n \in \mathbb{N}$, it follows directly from Equation \ref{eq:sin_cos} that
\begin{align*}
