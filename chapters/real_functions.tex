...
\begin{definition}{Bounded Functions}{bounded_func}
	...
	\]
	We say that $f$ is \textbf{bounded} if it is both bounded from above and from below. Equivalently, $f$ is bounded if there exists $M > 0$ such that
	\[
		|f(x)| \leq M \qquad \forall x \in D.
	\]
\end{definition}
...
\begin{definition}{Sum and Product Notation}{sum_prod_notation}
	...
	\[
		\sum_{j=0}^{n} a_j = a_0 + a_1 + \hdots + a_n, \qquad \prod_{j=0}^{n}a_j =  a_0 \cdot a_1 \cdot \hdots \cdot a_n.
	\]
...
\begin{proof}
	...
	\]
	exists.
	We now use the continuity of $f$ at $\bar{x}$ to show that $f(\bar{x}) = c$.
	
	Since $\bar{x}$ is the supremum of $X$, for each $n \geq 0$, we can find a point $x_n \in X \cap [\bar{x} - 2^{-n}, \bar{x}]$.
...
\begin{definition}{Sequences of Functions}{seq_func}
	A \textbf{sequence} of real-valued on a subset $D \subseteq \mathbb{R}$ is a family of functions $f_n: D \to \mathbb{R}$ indexed by $\mathbb{N}$. The function $f_n$ is called the n-th \textbf{element} of the sequence. One often writes $(f_n)_{n \in \mathbb{N}}$, $(f_n)_{n=0}^{\infty}$, or $(f_n)_{n\geq 0}$ for a sequence of functions.
\end{definition}

\begin{definition}{Pointwise Convergence}{ptwise_conv}
