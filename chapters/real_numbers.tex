...
\begin{proof}
	...
	3. By 2. we know that $-(a \cdot b) = a \cdot (-b)$. Hence, recalling Equation \eqref{eq:inverse_element},
	\[
		a \cdot b = -(a \cdot (-b)).
	\]
...
\begin{lemma}{Ordered Field: Basic Consequences}{consequences_ordered_field}
	...
	\begin{enumerate}
		...
		\item[(b)] If $x < y$ and $y \leq z$, then $x < z$. (Analogously, $x \leq y \text{ and } y < z \text{ imply } x < z$.)
...
\begin{lemma}{Absolute Value and Sign: Basic Properties}{prop_abs_sign}
	...
	\begin{enumerate}
		...
		\item[(b)] $|x| \geq 0$, and $|x| = 0$ if and only if $x = 0$ (by Trichotomy Lemma \ref{lem*consequences_ordered_field}).
...
\begin{definition}{Union and Intersection of several Sets}{union_int_sets}
	...
	\[
		\bigcup_{A \in \mathcal{A}} A = \{x \;|\; \exists A \in \mathcal{A} : x \in A\}, \quad \bigcap_{A \in \mathcal{A}} A = \{x \;|\; \forall A \in \mathcal{A} : x \in A\}.
	\]
...
\begin{lemma}{Cauchy-Schwarz Inequality}{cauchy_schwarz_ineq}
...
\begin{proof}
	...
	\[
		|x_1x_2 +y_1y_2| \leq |z||w|.
	\]
	Since $x \leq |x|$ for all $x \in \mathbb{R}$, we obtain Equation \eqref{eq:cauchy_schwarz_ineq}.\qedhere
\end{proof}
...
\begin{definition}{Bounded Sets, Maxima and Minima}{bound_min_max}
...
The archimedean principle states that for every real number $x \in \mathbb{R}$ there exists an integer $n$ greater than $x$. The following theorem, proved using the existence of suprema (and implicitly the completeness axiom), gives a precise formulation of this principle.
